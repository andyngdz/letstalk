...
RoomManager.prototype.update = function(roomId, options, cb) {
    ...
    Room.findById(roomId, function(err, room) {
        ...
        console.info(options.user.id, room.owner.equals(options.user.id), room.owner);
        if(!room.owner.equals(options.user.id)) {
            return cb('Only owner can change this room.');
        }
        ...
        getParticipants(room, options, function(err, participants) {
            ...
            room.background = options.background;
...
RoomManager.prototype.archive = function(roomId, options, cb) {
    var Room = mongoose.model('Room');
    ...
    Room.findById(roomId, function(err, room) {
        ...
        if(!room.owner.equals(options.user.id)) {
            return cb('Only owner can archive this room.');
        }
