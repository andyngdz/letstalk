...
function(window, $, _) {
    ...
    window.LCB.RoomView = Backbone.View.extend({
        ...
        states: {
            isFirstTimeInitScrollBar: false
        },
        ...
        goodbye: function() {
            toastr.warning('"' + this.model.get('name') + '" has been archived.', 'warning', 'Archived!');
        },
        updateMeta: function() {
        ...
        showEditRoom: function(e) {
            ...
            var $modal = this.$('.lcb-room-edit'),
                ...
                $background = $modal.find('input[name="background"]'),
            ...
            $background.val(this.model.get('background'));
            $password.val('');
            $confirmPassword.val('');
        ...
        archiveRoom: function(e) {
            ...
            $.confirm({
                title: 'Do you really want to archive "' +
                    this.model.get('name') + '"?',
                content: 'You will not be able to open it!',
                confirm: function() {
        ...
        addMessage: function(message) {
            ...
            this.formatMessage($text.html(), function(text) {
                $text.html(text);
                var texts, thumbnails, videos;
                // Video? Video? I like it!
                if (that.isVideo(message.text)) {
                    $text.empty();
                    $text.html(that.embedVideo(message.text));
                }

                $html.find('time').updateTimeStamp();

                if (!message.fragment) {
                    if (that.lastMessageOfOwer !== undefined && that.lastMessageOfOwer !== null) {
                        texts = that.lastMessageOfOwer.find('.messages').find('.lcb-message-text');
                        thumbnails = texts.find('.thumbnail').length;
                        videos = texts.find('iframe').length;
                        if (texts.length === (thumbnails + videos)) {
                            that.lastMessageOfOwer.find('.messages').addClass('no-background');
                        }
                    } else {
                        texts = $html.find('.messages').find('.lcb-message-text');
                        thumbnails = texts.find('.thumbnail').length;
                        videos = texts.find('iframe').length;
                        if (texts.length === (thumbnails + videos)) {
                            $html.find('.messages').addClass('no-background');
                        }
                    }
                    that.$messages.append($html);
                    that.lastMessageOfOwer = $html;
                } else {
                    // Append continuous Hahaha!
                    that.lastMessageOfOwer.find('.messages').append($html.find('.lcb-message-text'));
                }
        ...
        embedVideo: function(text) {
            var id = this.getYoutubeId(text);
            var y = '<iframe data-type="youtube"  src="https://www.youtube.com/embed/{0}?rel=0" frameborder="0" allowfullscreen></iframe>';
            return y.replace('{0}', id);
        },
