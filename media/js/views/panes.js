...
+function(window, $, _, ReactDOM, React) {
    ...
    window.LCB.TabsView = Backbone.View.extend({
        ...
        switch: function(id) {
            ...
            this.$lcbPanes = $('.lcb-panes');
            this.$lcbEntry = $('.lcb-entry');
            this.$listChildParanes = this.$lcbPanes.children();
            this.$listChildParanes.css('display', 'none').hide().filter('[data-id="' + id + '"]').css('display', 'block').fadeIn();
            if(id !== 'list'){
              this.$lcbEntry.css('display', 'block');
            }
            else{
              this.$lcbEntry.hide();
            }
            // Trigger input message to calculate height
            $('textarea.lcb-entry-input').trigger('input');
            $('textarea.lcb-entry-input').css('width', '').closest('.lcb-entry').css('width', '');
        },
        leave: function(e) {
            e.preventDefault();
    ...
    window.LCB.PanesView = Backbone.View.extend({
        ...
        add: function(room) {
            ...
            // this.extraFeature(this.views[room.id].$el);
            this.$el.append(this.views[room.id].$el);
        },
        extraFeature: function($el) {
          //Add more feature before room already initialized
          //Add voice-recorder
          ReactDOM.render(
            React.createElement(Voice),
            $el.find('.additional')[0]
          );
        },
        remove: function(id) {
            if (!this.views[id]) {
