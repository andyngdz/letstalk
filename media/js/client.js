...
(function(window, $, _) {
    ...
    Client.prototype.createRoom = function(data) {
        ...
        this.socket.emit('rooms:create', room, function(room) {
            if (room && room.errors) {
                toastr.error('Room slugs can only contain lower case letters, numbers or underscores!', 'Unable to create room!')
    ...
    Client.prototype.archiveRoom = function(options) {
        this.socket.emit('rooms:archive', options, function(data) {
            if (data !== 'No Content') {
                toastr.error('Unable to archive this room!', 'Unable to Archive!');
            }
        });
    };
